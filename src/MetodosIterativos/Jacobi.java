    ...
    private final void run() {
        ...
        while (!stop) {
            ...
            for (int i = 0; i < matrizAumentada.length; i++) { //LINHAS
                novoValor = new BigDecimal(matrizAumentada[i][matrizAumentada.length - 1]);
            ...
            for (int i = 0; i < matrizAumentada.length; i++) { //LINHAS
                ...
                for (int j = 0; j < matrizAumentada[i].length - 1; j++) { //COLUNAS
                    novoValor = novoValor.add(BigDecimal.valueOf(matrizAumentada[i][j] * valoresFinais[j].doubleValue()));
                }

                if (diferenca(novoValor, BigDecimal.valueOf(matrizAumentada[i][matrizAumentada.length - 1])).doubleValue() > 0.00001) {
                    stop = false;
                    break;
                }
            }
